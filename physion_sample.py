...
def main(args):
    ...
    #vae_num=sum(p.numel() for p in vae.parameters())
    #print(vae_num)
    ...
    if args.condition_frames > 0:
        ...
        with torch.no_grad():
            #cx=torch.zeros_like(condition_x)
            #_=vae.encode(cx).latent_dist.sample().mul_(0.18215)
            #start=torch.cuda.Event(enable_timing=True)
            #end=torch.cuda.Event(enable_timing=True)
            #start.record()
            #print(condition_x.shape)
    ...
    #start=torch.cuda.Event(enable_timing=True)
    #end=torch.cuda.Event(enable_timing=True)
    #_=diffusion.p_sample_loop(sample_fn,z.shape,torch.zeros_like(z),clip_denoised=False,model_kwargs=model_kwargs,progress=True,device=device,condition_frames=torch.zeros_like(condition_x))
    #start.record()
    ...
    #start=torch.cuda.Event(enable_timing=True)
    #end=torch.cuda.Event(enable_timing=True)
    #sa=torch.zeros_like(samples)
    #_ = vae.decode(sa).sample
    #start.record()
